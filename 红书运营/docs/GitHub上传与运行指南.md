...
**若你的远程仓库 clone 后第一层只有子目录「红书运营」**：这是多包了一层，**项目根 = `红书运营/` 里面**（即与 `CLAUDE.md` 同级）；协作者务必先 `cd 红书运营`，隐藏目录 **`.secrets`** 也在这一层里（访达需 ⌘⇧. 显示隐藏文件）。
...
**三段式心理路径**（要装什么、Key 放哪、路径相对哪一层）：请先读 **[`协作者从clone到第一句话.md`](协作者从clone到第一句话.md)**，再按需深入下面的步骤与 **[`上手运行指南.md`](上手运行指南.md)**。
...
确认当前目录下有 **`CLAUDE.md`**。可选：执行 `./routines/bootstrap_after_clone.sh` 预建常用目录。
...
| [`docs/协作者复制即用-跑通步骤.md`](协作者复制即用-跑通步骤.md) | **发给对方**：按编号步骤跑通（Web / 本机网关） |
| [`docs/协作者从clone到第一句话.md`](协作者从clone到第一句话.md) | 迷茫/配置/运行三段式；无 package 声明与路径约定 |
